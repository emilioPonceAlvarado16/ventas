import React from 'react'

export default function modalAlert(props) {
    const classSelected = `f-alert-${props.type}`;
    const messageSelected = props.message || "Alert placeholder";
    const details = props.detalles || "Detalles";
    //information, danger, error, success
  return (
    <div className="f-alert-large">
    <div className={classSelected}> 
      <div className="f-alert-icon w-embed"><svg width="24" height="24"  fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M12 22C6.477 22 2 17.523 2 12C2 6.477 6.477 2 12 2C17.523 2 22 6.477 22 12C22 17.523 17.523 22 12 22ZM12 20C14.1217 20 16.1566 19.1571 17.6569 17.6569C19.1571 16.1566 20 14.1217 20 12C20 9.87827 19.1571 7.84344 17.6569 6.34315C16.1566 4.84285 14.1217 4 12 4C9.87827 4 7.84344 4.84285 6.34315 6.34315C4.84285 7.84344 4 9.87827 4 12C4 14.1217 4.84285 16.1566 6.34315 17.6569C7.84344 19.1571 9.87827 20 12 20V20ZM11.003 16L6.76 11.757L8.174 10.343L11.003 13.172L16.659 7.515L18.074 8.929L11.003 16Z" fill="currentColor"></path>
        </svg></div>
    </div>
    <div className="f-alert-content">
      <div className="f-paragraph-regular f-text-weight-medium">{messageSelected}</div>
      <div className="f-paragraph-small-5 f-text-color-gray-500">{details}</div>
      <div className="f-alert-line"></div>
      <div className="f-alert-button-wrapper">
        <a href="#" className="f-alert-button">Accept</a>
        <a href="#" className="f-alert-secondary">Learn More</a>
      </div>
    </div>
  </div>
  )
}
